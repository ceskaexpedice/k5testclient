<xsl:stylesheet  version="1.0"
            ...
            <table width="100%">
                <xsl:for-each select="//doc" >
                    ...
                    <tr>
                        ...
                        <xsl:attribute name="data-pid_path"><xsl:value-of select="./arr[@name='pid_path']/str"/></xsl:attribute>
                        <xsl:attribute name="data-model_path"><xsl:value-of select="./arr[@name='model_path']/str"/></xsl:attribute>
    ...
    <xsl:template name="doc">
        ...
        <xsl:variable name="imagepid" >
            <xsl:choose>
                <xsl:when test="contains($pid, '/@')">api/item/<xsl:value-of select="substring-before($pid, '/@')"/>/thumb</xsl:when>
                <xsl:otherwise>api/item/<xsl:value-of select="$pid" />/thumb</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <td class="resultThumb" valign="top">
            <a>
                <xsl:attribute name="href">javascript:changeSelect('<xsl:value-of select="$pid"/>', '<xsl:value-of select="./arr[@name='model_path']/str"/>');</xsl:attribute>
            <img border="1" height="96"><xsl:attribute name="id">img_<xsl:value-of select="$pid"/></xsl:attribute>
